...
class CoffeeMachinesRouter:
    ...
    def router(self):
        ...
        def create_coffee_machine(
            coffee_machines_model: CoffeeMachinesModel,
            response: Response,
            ...
            return self.__coffee_machines_domain.create_coffee_machine(
                coffee_machines_model
            )

        @api_router.get("/get/{coffee_machine_uid}")
        def get_coffee_machine(coffee_machine_uid: str):
            try:
                return self.__coffee_machines_domain.get_coffee_machine(
                    coffee_machine_uid
                )
            except KeyError:
        ...
        def update_coffee_machine(
            ...
            if auth_result.get("status"):
                raise HTTPException(status_code=401, detail="Bad auth token")
            return self.__coffee_machines_domain.update_coffee_machine(
                coffee_machines_model
            )

        @api_router.delete("/delete/{coffee_machine_uid}")
        def delete_coffee_machine(
            coffee_machine_uid: str,
            response: Response,
            token: str = Depends(token_auth_scheme),
        ):
            auth_result = VerifyToken(token.credentials).verify()
            ...
            if auth_result.get("status"):
                raise HTTPException(status_code=401, detail="Bad auth token")
            return self.__coffee_machines_domain.delete_coffee_machine(
                coffee_machine_uid
            )
